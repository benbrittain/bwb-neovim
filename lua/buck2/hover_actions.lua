...
local function close_hover()
  buck2.utils.close_win(M._state.winnr)
end
...
function M.handler(_, result, ctx)
  ...
  local bufnr, winnr = util.open_floating_preview(
    markdown_lines,
    "markdown",
    vim.tbl_extend("keep", buck2.config.options.tools.hover_actions, {
      focusable = true,
  ...
  if buck2.config.options.tools.hover_actions.auto_focus then
...
function M.hover_actions()
  buck2.utils.request(0, "textDocument/hover", get_params(), M.handler)
end
